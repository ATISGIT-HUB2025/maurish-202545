(function ($) {
    ...
    if ($(".infiniteslide").length > 0) {
      $(".infiniteslide").each(function () {
        ...
        if ($(this).closest("body").hasClass("rtl")) {
          if ($this.data("style") === "left") {
            style = "right";
          } else {
            if ($this.data("style") === "right") {
              style = "left";
            } else {
              style = "right";
            }
      ...
      if (productCount === 0) {
        $wishlistInner.append(`
          <div class="tf-wishlist-empty text-center">
            ...
            <img  src="images/banner/empty-wishlist.jpg" alt="empty-wishlist" class="img-fluid" style="width: 100%; height: 100% ; object-fit: contain" />
            </div>

            <p class="text-md text-noti">No product were added to the wishlist.</p>
            <a href="product-list.html" class="tf-btn animate-btn btn-back-shop">Back to Shopping</a>
          </div>
        `);
      } else {
    ...
    $(".wrap-pos-nav").each(function () {
      ...
      if (imageBlog.length) {
        var contentHeight = $(this)
          .find(".blog-item-v2 .entry-content")
          .outerHeight();
      ...
      if (imageProduct.length) {
        var contentHeight = $(this)
          .find(".card-product .card-product-info")
          .outerHeight();
        var newTop = `calc(50% - ${contentHeight / 2}px)`;
        $(this).find(".nav-swiper").css("top", newTop);
      }
  ...
  var blockForm = function () {
    $(".form-newsletter").submit(function (e) {
      ...
      $(".form-newsletter").append(
        '<p class="mt_10 feedback-message text-sm">Order Track successfully!</p>'
      );
    });
  };
  ...
  var handleResultIP = function () {
    $(".box-ip-discount button").on("click", function (e) {
      ...
      if (discountCode === "") {
        $(".box-ip-discount").append(
          '<div class="discount-message text-sm error">Please enter a discount code.</div>'
        );
      } else {
        $(".box-ip-discount").append(
          '<div class="discount-message text-sm success">Discount code applied successfully!</div>'
        );
      }
